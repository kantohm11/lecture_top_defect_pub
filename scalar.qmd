...
When the transformation is trivial, i.e., $g=\id$, the operator $U_{g=\id}[W]$ should also be *trivial*. This implies that
$$
...
\end{aligned}
$${#eq-Ward-Takahashi}
where $W_i$ encloses the operators $\mathcal{O}_1 \cdots \mathcal{O}_i$, and at the last step we collapsed $U_g[W_N]$ towards the infinity (or whatever point if $M$ is compact). See also @fig-WardTakahashi.
:::

:::{.content-visible when-format="html"}

![Derivation of Ward-Takahashi identity @eq-Ward-Takahashi.](figures/tikz/WardTakahashi.svg){#fig-WardTakahashi width=400}

:::

:::{.content-visible when-format="pdf"}

![Derivation of Ward-Takahashi identity @eq-Ward-Takahashi.](figures/tikz/WardTakahashi.pdf){#fig-WardTakahashi width=400}
...
Although this statement for $\mathbb{Z}_k$ symmetry should also be familiar by, e.g., investigating perturbative expansion, it cannot be formally derived based on current operators, as a discrete symmetry does not have a current.
On the other hand, this method of topological operator applies to discrete symmetry that is not a reminisent of a continuous symmetry, an example of which we will see in @sec-Tdual.
That this method of topological operator formally leads to the Ward-Takahashi identity for both discrete and continuous symemtry signals its importance.
