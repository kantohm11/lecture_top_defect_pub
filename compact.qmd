...
$U^\text{shift}_\alpha$ and $U^\text{shift}_{\alpha+2\pi}$ can be connected with an invertible topological junction [^isomUalpha]:
$$
...
See also @fig-shift-isom

[](figures/){#fig-shift-isom}

[^isomUalpha]: In the language of category theory, they are isomorphic. In (higher) category theory it is important to distinguish that two objects are the *same* and that the two objects are *isomorphic*.
